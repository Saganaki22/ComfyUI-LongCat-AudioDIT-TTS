...
class AudioDiTTransformer(nn.Module):
    ...
    def __init__(self, config: AudioDiTConfig):
        ...
        self.blocks = nn.ModuleList(
            [AudioDiTBlock(config) for _ in range(config.dit_depth)]
        )
...
class AudioDiTVae(nn.Module):
    ...
    def encode(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        """
        w_dtype = next(self.encoder.parameters()).dtype
        if w_dtype != torch.float32:
            audio = audio.to(w_dtype)
        latents = self.encoder(audio)
        # VAE bottleneck runs in the same dtype as encoder output
    ...
    def decode(self, latents: torch.Tensor) -> torch.Tensor:
        ...
        w_dtype = next(self.decoder.parameters()).dtype
        if w_dtype != torch.float32:
            z = z.to(w_dtype)
